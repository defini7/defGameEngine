...
namespace def
{
	...
	public:
		...
		void Draw(int32_t x, int32_t y, const Pixel& p = WHITE);
		...
		template <typename T>
		void DrawGFX(vec2d_basic<T> pos, GFX* gfx, vec2d_basic<T> scale);
		void DrawGFX(int32_t x, int32_t y, GFX* gfx, float scale_x = 1.0f, float scale_y = 1.0f);

		template <typename T>
		void DrawPartialGFX(vec2d_basic<T> pos, vec2d_basic<T> fpos, vec2d_basic<T> fsize, GFX* gfx, vec2d_basic<T> scale);
		...
		template <typename T>
		void DrawString(vec2d_basic<T> pos, const std::string& text, const Pixel& p = WHITE, uint32_t scale = 1);
		void DrawString(int32_t x, int32_t y, const std::string& text, const Pixel& p = WHITE, uint32_t scale = 1);
	...
	void def::GameEngine::DrawString(int32_t x, int32_t y, const std::string& s, const Pixel& p, uint32_t scale)
	{
		int32_t sx = 0;
		int32_t sy = 0;
	...
	template<typename T>
	void GameEngine::DrawGFX(vec2d_basic<T> pos, GFX* gfx, vec2d_basic<T> scale)
	{
		DrawGFX(pos.x, pos.y, gfx, scale.x, scale.y);
	}

	template<typename T>
	void GameEngine::DrawPartialGFX(vec2d_basic<T> pos, vec2d_basic<T> fpos, vec2d_basic<T> fsize, GFX* gfx, vec2d_basic<T> scale)
	{
	...
	template<typename T>
	void GameEngine::DrawString(vec2d_basic<T> pos, const std::string& text, const Pixel& p, uint32_t scale)
	{
		DrawString(pos.x, pos.y, text, p, scale);
	}
